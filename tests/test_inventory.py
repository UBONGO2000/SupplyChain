...
"""
from conftest import auth_headers, make_product, make_inventory
...
class TestInventoryAdjust:
    ...
    def test_negative_adjustment_below_zero_rejected_and_state_unchanged(
        ...
        inv = make_inventory(db_session, warehouse, product, quantity=10, reserved_quantity=0)
    ...
    def test_adjust_nonexistent_inventory_record_returns_404(self, client, staff_user, warehouse, product):
        resp = client.post(
            f"/api/inventory/adjust?warehouse_id={warehouse.id}&product_id={product.id}",
            json={"adjustment": 5},
            headers=auth_headers(staff_user),
    ...
    def test_duplicate_inventory_record_rejected(
        ...
        resp = client.post(
            "/api/inventory",
            json={"warehouse_id": warehouse.id, "product_id": product.id, "quantity": 5},
            headers=auth_headers(staff_user),
        )

        assert resp.status_code == 400

    def test_staff_can_adjust_inventory(self, client, staff_user, db_session, warehouse, product):
        make_inventory(db_session, warehouse, product, quantity=20)
    ...
    def test_viewer_cannot_adjust_inventory(self, client, viewer_user, db_session, warehouse, product):
        make_inventory(db_session, warehouse, product, quantity=20)
    ...
    def test_viewer_cannot_create_inventory(self, client, viewer_user, warehouse, product):
        resp = client.post(
            "/api/inventory",
            json={"warehouse_id": warehouse.id, "product_id": product.id, "quantity": 20},
            headers=auth_headers(viewer_user),
        )
